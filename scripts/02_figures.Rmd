...
```{r}
# Load packages and datasets for making figures:
...
figure5 <- rs_sites_4p %>%
  ...
  pivot_longer(c(-site, -no_plots, -tree_density, -loreys_height, -basal_area, -agb, -is_unexpected ),
               names_to = "covariates",
               values_to = "value") %>%
  mutate("AGB (Mg/ha)" = log1p(agb),
  "Basal area (m/ha)" = sqrt(basal_area),
  "Lorey's height (m)" = sqrt(loreys_height),
  "Tree density (tree/ha)" = sqrt(tree_density)) %>%
  pivot_longer(c("AGB (Mg/ha)", "Basal area (m/ha)", "Lorey's height (m)", "Tree density (tree/ha)"),
               names_to = "forest_str",
               values_to = "f_str_values") %>%
  filter(covariates == "Elevation (m asl)" | 
         covariates == "NDVIa S.D." |
...
figure5 <- figure5 + facetted_pos_scales(
    y = list(
    forest_str == "AGB (Mg/ha)" ~ scale_y_continuous(breaks = c(0, 2.397895, 4.615121,5.303305, 5.993961,6.398595, 6.685861),
                                             labels = c("0", "10", "100", "200", "400","600", "800")),
    forest_str == "Basal area (m/ha)" ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 4.472136, 6.324555, 7.745967, 8.944272),
                                                    labels = c("0", "5", "10", "20", "40", "60", "80")),
    forest_str == "Lorey's height (m)" ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 3.872983, 4.472136, 5, 5.477226),
                                                        labels = c("0", "5", "10", "15", "20", "25", "30")),
    forest_str == "Tree density (tree/ha)" ~ scale_y_continuous(breaks = c(0, 10, 22.36068, 31.62278, 38.72983, 44.72136, 50),
                                                      labels = c("0", "100", "500", "1000", "1500", "2000", "2500"))
  )
)

ggsave("FIGURE5.png", figure5, width=18, height=14, units="cm", dpi = "print")

## Figure 5 with plots

figure5_plots_df <- rs_shorter_4p %>%
  mutate('"Elevation (m asl)"' = as.numeric(altitude),
         '"Slope (degrees)"' = as.numeric(slope),
         '"No. breaks"' = as.numeric(number_breaks),
         '"NDVIa S.D."' = as.numeric(ndvi_annual_sd),
         '"NDWIa min"' = as.numeric(ndwi_annual_min)) %>%
  pivot_longer(c(-site, -no_plots, -plot_id, -tree_density, -loreys_height, -basal_area_ha, -agb_plot_ha),
               names_to = "covariates",
               values_to = "value") %>%
  # mutate("AGB (Mg/ha)" = log1p(agb_plot_ha),
  #        "Basal area (m^2/ha)" = sqrt(basal_area_ha),
  #        "Lorey's height (m)" = sqrt(loreys_height),
  #        "Tree density (tree/ha)" = sqrt(tree_density)) %>%
  mutate(agb = log1p(agb_plot_ha),
         ba = sqrt(basal_area_ha),
         lh = sqrt(loreys_height),
         td = sqrt(tree_density)) %>%
  pivot_longer(c(agb, ba, lh, td),
               names_to = "forest_str",
               values_to = "f_str_values") %>%
  filter(covariates == '"Elevation (m asl)"' | 
         covariates == '"Slope (degrees)"' |
         covariates == '"No. breaks"' |
         covariates == '"NDVIa S.D."' |
         covariates == '"NDWIa min"')

figure5_plots_df$forest_str <- factor(figure5_plots_df$forest_str,
                                labels = c((bquote('AGB ('*"Mg"~ha^-1*')')),
                                           (bquote('Basal area ('*"m"^2 ~ha^-1*')')),
                                           (bquote('"Lorey´s height (m)"')),
                                           (bquote('Tree density ('*"tree"~ha^-1*')'))))


figure5_plots <- figure5_plots_df %>%
  ggplot(aes(value, f_str_values)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement.x = "bottom",
        strip.placement.y = "left")

figure5_plots <- figure5_plots + facetted_pos_scales(
    y = list(
    forest_str == (bquote('AGB ('*"Mg"~ha^-1*')')) ~ scale_y_continuous(breaks = c(0, 2.397895, 4.615121,5.303305, 5.993961,6.398595, 6.685861),
                                             labels = c("0", "10", "100", "200", "400","600", "800")),
    forest_str == (bquote('Basal area ('*"m"^2 ~ha^-1*')')) ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 4.472136, 6.324555, 7.745967, 8.944272),
                                                    labels = c("0", "5", "10", "20", "40", "60", "80")),
    forest_str == (bquote('"Lorey´s height (m)"')) ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 3.872983, 4.472136, 5, 5.477226),
                                                        labels = c("0", "5", "10", "15", "20", "25", "30")),
    forest_str == (bquote('Tree density ('*"tree"~ha^-1*')')) ~ scale_y_continuous(breaks = c(0, 10, 22.36068, 31.62278, 38.72983, 44.72136, 50),
                                                      labels = c("0", "100", "500", "1000", "1500", "2000", "2500"))
  )
)

ggsave("FIGURE5_revised.tiff", figure5_plots, width=19, height=15, units="cm", dpi = 600)
```
...
figure6_df$forest_str <- factor(figure6_df$forest_str,
                                labels = c((bquote('AGB ('*"Mg"~ha^-1*')')),
                                           (bquote('Basal area ('*"m"^2 ~ha^-1*')')),
                                           (bquote('"Lorey´s height (m)"')),
                                           (bquote('Tree density ('*"tree"~ha^-1*')'))))
...
ggsave("FIGURE3_revised.tiff", figure6, width=18, height=12, units="cm", dpi = 600)
...
# figure7_db <- plots_cf_rs %>%
#   mutate("AGB (Mg/ha)" = log1p(agb_plot_ha),
#          "Basal area (m*m/ha)" = sqrt(basal_area_ha),
#          "Lorey's height (m)" = sqrt(loreys_height),
#          "Tree density (tree/ha)" = sqrt(tree_density)) %>%
#   mutate(is_break = ifelse(number_breaks == 0, "no break", "break")) %>%
...
figure7_db <- plots_cf_rs %>%
  ...
  mutate(is_break = ifelse(number_breaks == 0, "no break", "break")) %>%
  filter(is_break == "break") %>%
  select(agb, ba, lh, td, age) %>%
  pivot_longer(c(agb, ba, lh, td),
               names_to = "forest_str",
               values_to = "value")

figure7_db$forest_str <- factor(figure7_db$forest_str,
                                labels = c((bquote('AGB ('*"Mg"~ha^-1*')')),
                                           (bquote('Basal area ('*"m"^2 ~ha^-1*')')),
                                           (bquote('"Lorey´s height (m)"')),
                                           (bquote('Tree density ('*"tree"~ha^-1*')'))))
...
figure7 <- figure7_db %>%
  # pivot_longer(c("AGB (Mg/ha)", "Basal area (m*m/ha)", "Lorey's height (m)", "Tree density (tree/ha)"),
  #              names_to = "forest_str",
  #              values_to = "value") %>%
  # pivot_longer(c(agb, ba, lh, td),
  #              names_to = "forest_str",
...
# (c("AGB (Mg/ha)" = (bquote('AGB ('*"Mg"~ha^-1*')')),
#                           "Basal area (m*m/ha)" = (bquote('Basal area ('*"m"^2 ~ha^-1*')')),
#                           "Lorey's height (m)" = "Lorey's height (m)",
#                           "Tree density (tree/ha)" = (bquote('Tree density ( '*"tree"~ha^-1*')'))))

# figure7 <- figure7 + facetted_pos_scales(
#     y = list(
#     forest_str == "AGB (Mg/ha)" ~ scale_y_continuous(breaks = c(0, 2.397895, 4.615121,5.303305, 5.993961,6.398595, 6.685861),
#                                              labels = c("0", "10", "100", "200", "400","600", "800")),
#     forest_str == "Basal area (m*m/ha)" ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 4.472136, 6.324555, 7.745967, 8.944272),
#                                                     labels = c("0", "5", "10", "20", "40", "60", "80")),
#     forest_str == "Lorey's height (m)" ~ scale_y_continuous(breaks = c(0, 2.236068, 3.162278, 3.872983, 4.472136, 5, 5.477226),
#                                                         labels = c("0", "5", "10", "15", "20", "25", "30")),
#     forest_str == "Tree density (tree/ha)" ~ scale_y_continuous(breaks = c(0, 10, 22.36068, 31.62278, 38.72983, 44.72136, 50),
...
figure7 <- figure7 + facetted_pos_scales(
    y = list(
    forest_str == (bquote('AGB ('*"Mg"~ha^-1*')')) ~ scale_y_continuous(breaks = c(0,
                                                                                   2.397895,
                                                                                   4.615121,5.303305,
                                                                                   5.993961,
                                                                                   6.398595,
...
ggsave("FIGURE4_revised.tiff", figure7, width=18, height=4.5, units="cm", dpi = 600)
...
summary(lm(figure7_db$AGB ~ figure7_db$age))
summary(lm(figure7_db$`Basal area` ~ figure7_db$age))
summary(lm(figure7_db$`Tree density` ~ figure7_db$age))
summary(lm(figure7_db$`Lorey's height` ~ figure7_db$age))
```
